...
def add_address():
    with session_scope() as sess:
        wms = sess.query(WaiMai).all()
        driver.get("https://www.meituan.com/")
        for item in wms:
            if not item.address and not item.openTime:
                res = {}
                driver.get("https://waimai.meituan.com/home/{}".format(item.geoArea))
                time.sleep(0.5)
                driver.get(item.url)
                ...
                try:
                    ...
                    print("{} {}".format(item.url, e))
                r = driver.page_source
                soup = BeautifulSoup(r, "lxml")
                try:
                    div = soup.find("div", class_="rest-info-down-wrap")
                    timep = div.find("div", class_="clearfix sale-time")
                    if timep.strip():
                        timep = timep.text.split()[-1]
                        item.openTime = timep.strip()
                        print(timep.strip())
                except:
                    pass
                try:
